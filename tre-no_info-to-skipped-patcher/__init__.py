...
def main(timer: func.TimerRequest, blobout: func.Out[bytes]):
  ...
  url = 'https://tre.mattersoft.fi/api/gtfsrealtime/v1.0/feed/tripupdate'
  user_name = os.getenv('TRE_USER')
  password = os.getenv('TRE_PASSWORD')
  res = requests.get(url, verify=True, auth=HTTPBasicAuth(user_name, password))
  if res.status_code != 200:
    logging.error(f"----- tampere tripupdate download failed")
  ...
  logging.info(f"----- tampere blob updated")
